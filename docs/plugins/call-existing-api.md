...
fetch('https://frobs.example.com/v1/list')
...
proxy:
  '/frobs':
    target: 'http://frobs.example.com/v1'
    changeOrigin: true
    pathRewrite:
      '^/proxy/frobs/': '/'
```
...
system. The above mentioned proxy is actually one such plugin. When you are in
need of a more involved integration than just direct access to an API, or if you
need to hold state, you may want to make such a plugin.
...
that want to resolve a lot of sparse data from a larger list.
